...
#if (__LONG_MAX__==9223372036854775807L) || defined(_LONGLONG)
#define WORD_LARGE long long
#define WORD_NORMAL long
#define NIBBLES_PER_WORD 16
#else
#define WORD_LARGE long
#define WORD_NORMAL int
...
int vsnprintf(char *str, int size, const char *fmt, va_list ap)
{
    ...
    int i, c, sign, signch, numdigits, numpr = 0, someflag;
    unsigned long uval, uval2, base;
#if defined(USE_FP)
    double fval;
#endif
...
#define RIGHTPAD(num) do {				\
	...
	if ( *fmt == '.' ) {
	    if ( (c = *++fmt) == '*' ) {
		precision = va_arg( ap, WORD_NORMAL );
		fmt++;
	    } else if ( c >= '0' && c <= '9' ) {
		precision = 0;
		while ( (c = *fmt++) >= '0' && c <= '9' ) {
	...
	case 'b':
	    uval = f_long ? va_arg( ap, WORD_LARGE ) : va_arg( ap, WORD_NORMAL );
	    base = 2;
	    digits = "01";
	    goto Print_unsigned;
	...
	case 'p':
	    precision = width = NIBBLES_PER_WORD;
	    f_alternate = 1;
	case 'x':
	    uval = f_long ? va_arg( ap, WORD_LARGE ) : va_arg( ap, WORD_NORMAL );
	    base = 16;
...

{
    ...
    if ((fmt==NULL) || (dest==NULL))
      return 0;

    va_start(ap, fmt);
    r = vsnprintf(dest, 9999, fmt, ap);
    va_end(ap);
